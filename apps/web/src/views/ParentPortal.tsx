...
export default function ParentPortal() {
  ...
  const portalContext = {
    homework: Array.isArray(context?.homework) ? context.homework : [],
    ...
    appointments: Array.isArray(context?.appointments) ? context.appointments : [],
  };

  useEffect(() => { apiRequest<any[]>('/parent/children').then((items) => { const list = Array.isArray(items) ? items : []; setChildren(list); setSelectedId(list[0]?.learner.id || ''); }).catch((e) => notifyAction(e.message, 'error')).finally(() => setLoading(false)); }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (endpoint && (selectedId || ['orders','tickets'].includes(active))) apiRequest<any[]>(endpoint).then((items) => setSectionData(Array.isArray(items) ? items : [])).catch((e) => notifyAction(e.message, 'error'));
  }, [active, selectedId]);
  ...
  return <div className="flex min-h-screen bg-gray-50">
    ...
    <main className="min-w-0 flex-1 p-4 md:p-7"><div className="mx-auto max-w-6xl space-y-6">
      ...
      {selected && context && active === 'dashboard' && <><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Homework" value={portalContext.homework.length}/><Metric label="Attendance records" value={portalContext.attendance.length}/><Metric label="Assessments" value={portalContext.assessments.length}/><Metric label="Announcements" value={portalContext.announcements.length}/></div><div className="grid gap-5 lg:grid-cols-2"><Panel title="Upcoming homework"><Items items={portalContext.homework} empty="No published homework."/></Panel><Panel title="Recent announcements"><Items items={portalContext.announcements} empty="No class announcements."/></Panel></div></>}
      {selected && context && ['homework','attendance','assessments','announcements','appointments'].includes(active) && <Panel title={sections.find(([key]) => key === active)?.[1] as string}><Items items={portalContext[active as keyof typeof portalContext]} empty={`No ${active} records are available for this child.`}/></Panel>}
