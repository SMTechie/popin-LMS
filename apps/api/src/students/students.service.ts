...
export class StudentsService {
  ...
  async list(search?: string, status?: string, gradeId?: string, classId?: string) {
    const tenant = await this.tenants.getDefaultTenant();
    const where: Prisma.LearnerProfileWhereInput = {
      tenantId: tenant.id,
  ...
  private async ensureNoDuplicate(dto: CreateStudentDto, ignoreId?: string) {
    const tenant = await this.tenants.getDefaultTenant();
    const or: Prisma.LearnerProfileWhereInput[] = [];
    if (dto.identityNumber) or.push({ identityNumber: dto.identityNumber });
    if (dto.dateOfBirth) or.push({ firstName: { equals: dto.firstName, mode: "insensitive" }, lastName: { equals: dto.lastName, mode: "insensitive" }, dateOfBirth: new Date(dto.dateOfBirth) });
    if (!or.length) return;
    const duplicate = await this.prisma.learnerProfile.findFirst({ where: { tenantId: tenant.id, ...(ignoreId ? { id: { not: ignoreId } } : {}), OR: or } });
    if (duplicate) throw new BadRequestException(`Possible duplicate student: ${duplicate.studentNumber}`);
  }

  private async studentNumber(tenantCode: string) {
  ...
  async create(dto: CreateStudentDto, actorId: string) {
    ...
    if (dto.classId) {
      ...
      if (!target) throw new BadRequestException("Class does not belong to this school");
    }
    const learner = await this.prisma.learnerProfile.create({ data: {
      ...
      address: dto.address as Prisma.InputJsonValue | undefined, medicalNotes: dto.medicalNotes || null,
  ...
  async move(id: string, dto: MoveStudentDto, actorId: string) {
    const current = await this.get(id);
    const tenant = await this.tenants.getDefaultTenant();
  ...
  async bulkImport(dto: BulkImportStudentsDto, actorId: string) {
    const results: Array<{ row: number; status: string; studentNumber?: string; error?: string }> = [];
    for (let index = 0; index < dto.students.length; index++) {
      try { const item = await this.create(dto.students[index], actorId); results.push({ row: index + 1, status: "created", studentNumber: item.studentNumber }); }
      catch (error) { results.push({ row: index + 1, status: "rejected", error: error instanceof Error ? error.message : "Import failed" }); }
    }
    return { created: results.filter((r) => r.status === "created").length, rejected: results.filter((r) => r.status === "rejected").length, results };
  }

  async linkGuardian(learnerId: string, dto: LinkGuardianDto, actorId: string) {
    const learner = await this.get(learnerId);
    let guardian = await this.prisma.user.findUnique({ where: { email: dto.email.toLowerCase() } });
    if (guardian && !["PARENT", "EXTERNAL"].includes(guardian.userType)) throw new BadRequestException("This email belongs to a staff account");
    if (!guardian) {
  ...
  async updateGuardian(linkId: string, dto: UpdateGuardianLinkDto, actorId: string) {
  ...
  async unlinkGuardian(linkId: string, actorId: string) {
    const existing = await this.prisma.guardianStudentLink.findUnique({ where: { id: linkId } });
    if (!existing) throw new NotFoundException("Guardian link not found");
  ...
  async createTeacher(dto: CreateTeacherDto, actorId: string) {
    if (await this.prisma.user.findUnique({ where: { email: dto.email.toLowerCase() } })) throw new BadRequestException("Email already exists");
    const role = await this.prisma.role.findUnique({ where: { name: "Teacher" } });
    if (!role) throw new BadRequestException("Teacher role is not configured");
    const passwordHash = dto.temporaryPassword ? await bcrypt.hash(dto.temporaryPassword, 12) : null;
    const tenant = await this.tenants.getDefaultTenant();
    const teacher = await this.prisma.$transaction(async (tx) => {
      ...
      if (dto.assignments?.length) await tx.teacherClassAssignment.createMany({ data: dto.assignments.map((a) => ({ tenantId: tenant.id, teacherId: user.id, classId: a.classId, subject: a.subject, isPrimary: a.isPrimary ?? false })) });
      return user;
    });
  ...
  async createGrade(dto: CreateGradeDto, actorId: string) {
    const tenant = await this.tenants.getDefaultTenant();
    ...
    await this.audit.log({ actorId, action: "academic.grade.create", entity: "AcademicGrade", entityId: grade.id }); return grade;
  }
  async createClass(dto: CreateClassDto, actorId: string) {
    ...
    await this.audit.log({ actorId, action: "academic.class.create", entity: "SchoolClass", entityId: schoolClass.id }); return schoolClass;
  }
  async createSubject(dto: CreateSubjectDto, actorId: string) {
    ...
    await this.audit.log({ actorId, action: "academic.subject.create", entity: "AcademicSubject", entityId: subject.id }); return subject;
  }
  async assignSubject(classId: string, dto: AssignSubjectDto, actorId: string) {
    const tenant = await this.tenants.getDefaultTenant();
    ...
    await this.audit.log({ actorId, action: "academic.class.subject.assign", entity: "ClassSubject", entityId: link.id, data: { classId, subjectId: dto.subjectId } }); return link;
  }

  private permissionData(dto: Partial<LinkGuardianDto & UpdateGuardianLinkDto>) {
    const keys = ["viewFees", "payFees", "viewReports", "viewHomework", "receiveAnnouncements", "messageTeachers", "authorisePickup", "submitApplications", "bookAppointments", "viewHostel", "applyHostel", "viewHostelBilling", "submitHostelConcerns", "viewHostelMovement"] as const;
