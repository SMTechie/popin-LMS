...
import { SubjectPluginRegistry } from "./plugins/registry";

@Injectable()
export class TeacherAssistantService implements OnModuleInit {
  private registry = new SubjectPluginRegistry();
  ...
  private async owned(teacherId: string, assessmentId: string) {
    ...
    if (!assessment) throw new NotFoundException("Assessment not found or not assigned to this teacher");
    return assessment;
  }
  ...
  async get(teacherId: string, id: string) { return this.owned(teacherId, id); }

  async create(teacherId: string, dto: CreateAssistantAssessmentDto) {
    const { tenant, classIds, assignments } = await this.context(teacherId);
    if (!dto.classIds.length || dto.classIds.some((id) => !classIds.includes(id))) throw new ForbiddenException("Every selected class must be assigned to this teacher");
    if (!assignments.some((item) => dto.classIds.includes(item.classId) && item.subject.toLowerCase() === dto.subject.toLowerCase())) throw new ForbiddenException("Subject is not assigned to you for the selected classes");
    if (!dto.questions.length) throw new BadRequestException("At least one memorandum question is required");
    const memoTotal = dto.questions.reduce((sum, item) => sum + item.marks, 0);
    if (Math.abs(memoTotal - dto.totalMarks) > .001) throw new BadRequestException(`Memorandum marks (${memoTotal}) must equal total marks (${dto.totalMarks})`);
    const plugin = this.registry.resolve(dto.subject, dto.questions[0].questionType);
    if (!plugin) throw new BadRequestException("No deterministic marking plug-in supports this subject/question type");
    const assessment = await this.prisma.teacherAssistantAssessment.create({ data: {
      ...
      dueAt: dto.dueAt ? new Date(dto.dueAt) : null, uploads: dto.uploads as Prisma.InputJsonValue | undefined,
      settings: dto.settings as Prisma.InputJsonValue | undefined,
      questions: { create: dto.questions.map((question, index) => ({ questionNumber: question.questionNumber, questionType: question.questionType, expectedAnswers: question.expectedAnswers as Prisma.InputJsonValue, alternativeAnswers: question.alternativeAnswers as Prisma.InputJsonValue | undefined, marks: question.marks, tolerance: question.tolerance, decimalPrecision: question.decimalPrecision, requiredUnit: question.requiredUnit, caseSensitive: question.caseSensitive ?? false, ignoreWhitespace: question.ignoreWhitespace ?? true, partialMarkRules: question.partialMarkRules as Prisma.InputJsonValue | undefined, sortOrder: index })) }
  ...
  async uploadBatch(teacherId: string, assessmentId: string, dto: BatchUploadDto) {
    const assessment = await this.owned(teacherId, assessmentId);
    if (!dto.scripts.length || dto.scripts.length > 100) throw new BadRequestException("Upload between 1 and 100 scripts per batch");
    const created = await this.prisma.$transaction(async (tx) => {
      const scripts = [];
      for (const input of dto.scripts) {
        if (input.learnerId) {
          const learner = await tx.learnerProfile.findFirst({ where: { id: input.learnerId, classId: { in: assessment.classIds as string[] } } });
          if (!learner) throw new ForbiddenException("Selected learner is outside this assessment's classes");
        }
        const script = await tx.teacherAssistantScript.create({ data: { assessmentId, learnerId: input.learnerId || null, detectedStudentNumber: input.detectedStudentNumber || null, detectedStudentName: input.detectedStudentName || null, originalFiles: input.originalFiles as Prisma.InputJsonValue, extractedText: input.extractedText || null, status: "QUEUED" } });
  ...
  async updateExtractedText(teacherId: string, scriptId: string, dto: UpdateExtractedTextDto) {
    const script = await this.prisma.teacherAssistantScript.findUnique({ where: { id: scriptId }, include: { assessment: true } });
    if (!script || script.assessment.teacherId !== teacherId) throw new NotFoundException("Script not found");
    ...
    setTimeout(() => void this.processQueuedJobs(), 10); return updated;
  }

  async override(teacherId: string, scriptId: string, dto: OverrideResultsDto) {
    ...
    for (const item of dto.results) {
      ...
      if (!existing) throw new BadRequestException("Question result does not belong to this script");
      const question = await this.prisma.teacherAssistantQuestion.findUnique({ where: { id: existing.questionId } });
      if (!question || item.finalMarks > question.marks) throw new BadRequestException(`Override cannot exceed ${question?.marks ?? 0} marks`);
  ...
  async approve(teacherId: string, assessmentId: string, selection: ScriptSelectionDto) {
    const assessment = await this.owned(teacherId, assessmentId);
    const scripts = assessment.scripts.filter((item) => selection.scriptIds.includes(item.id));
    if (!scripts.length || scripts.length !== selection.scriptIds.length) throw new BadRequestException("One or more scripts are outside this assessment");
    if (scripts.some((item) => !["READY_FOR_REVIEW","NEEDS_REVIEW"].includes(item.status))) throw new BadRequestException("Only reviewed or exception scripts can be approved");
  ...
  async publish(teacherId: string, assessmentId: string, selection: ScriptSelectionDto) {
    const assessment = await this.owned(teacherId, assessmentId);
    const scripts = assessment.scripts.filter((item) => selection.scriptIds.includes(item.id));
    if (!scripts.length || scripts.some((item) => item.status !== "APPROVED")) throw new BadRequestException("Teacher approval is mandatory before publishing");
    ...
    if (!remaining) await this.prisma.teacherAssistantAssessment.update({ where: { id: assessmentId }, data: { status: "COMPLETE" } });
  ...
  async analytics(teacherId: string, assessmentId: string) {
    const assessment = await this.owned(teacherId, assessmentId);
    ...
    const middle = Math.floor(scores.length / 2); const median = !scores.length ? 0 : scores.length % 2 ? scores[middle] : (scores[middle - 1] + scores[middle]) / 2;
  ...
  private async processQueuedJobs() {
    ...
    try {
      for (let iteration = 0; iteration < 200; iteration++) {
        ...
        try {
          if (candidate.type === "OCR") await this.processOcr(candidate.scriptId!); else await this.processMark(candidate.scriptId!);
  ...
  private async processOcr(scriptId: string) {
    const script = await this.prisma.teacherAssistantScript.findUnique({ where: { id: scriptId }, include: { assessment: true } });
    if (!script) return;
    ...
    if (!extraction.text) {
      await this.prisma.teacherAssistantScript.update({ where: { id: scriptId }, data: { status: "NEEDS_REVIEW", ocrConfidence: extraction.confidence, needsReview: true, flags: ["OCR_TEXT_REQUIRED"] } }); return;
    }
    const identity = await this.detectStudent(script.assessment.classIds as string[], extraction.text, script.learnerId);
  ...
  private async processMark(scriptId: string) {
    ...
    if (!script?.editableText) return;
    await this.prisma.teacherAssistantScript.update({ where: { id: scriptId }, data: { status: "MARKING" } });
    let total = 0; let confidenceTotal = 0; const detectedAnswers: Record<string, string> = {}; const flags = Array.isArray(script.flags) ? [...script.flags as string[]] : [];
    for (const question of script.assessment.questions) {
      const detected = this.detectAnswer(script.editableText, question.questionNumber); detectedAnswers[question.questionNumber] = detected;
      const plugin = this.registry.resolve(script.assessment.subject, question.questionType);
      const decision = plugin ? plugin.mark(question as any, detected) : { awarded: 0, confidence: 0, rule: "No plug-in", explanation: "No marking plug-in supports this question." };
      const ocrConfidence = detected ? Number(script.ocrConfidence || .8) : .35; const markingConfidence = decision.confidence * ocrConfidence;
      if (markingConfidence < .7) flags.push(`LOW_CONFIDENCE_Q${question.questionNumber}`);
      await this.prisma.teacherAssistantQuestionResult.upsert({ where: { scriptId_questionId: { scriptId, questionId: question.id } }, update: { detectedAnswer: detected || null, expectedAnswer: question.expectedAnswers as Prisma.InputJsonValue, matchingRule: `${decision.rule}. ${decision.explanation}`, suggestedMarks: decision.awarded, finalMarks: decision.awarded, ocrConfidence, markingConfidence }, create: { scriptId, questionId: question.id, questionNumber: question.questionNumber, detectedAnswer: detected || null, expectedAnswer: question.expectedAnswers as Prisma.InputJsonValue, matchingRule: `${decision.rule}. ${decision.explanation}`, suggestedMarks: decision.awarded, finalMarks: decision.awarded, ocrConfidence, markingConfidence } });
      total += decision.awarded; confidenceTotal += markingConfidence;
    }
    const confidence = script.assessment.questions.length ? confidenceTotal / script.assessment.questions.length : 0; const needsReview = flags.length > 0 || !script.learnerId;
  ...
  private async extract(files: unknown, suppliedText?: string | null) {
    if (suppliedText?.trim()) return { text: suppliedText.trim(), confidence: .99 };
    const list = Array.isArray(files) ? files as Array<Record<string, any>> : [];
    const embedded = list.map((file) => file.text || file.extractedText || "").filter(Boolean).join("\n");
    if (embedded) return { text: embedded, confidence: .97 };
    if (process.env.OCR_SERVICE_URL && list.length) {
      const response = await fetch(process.env.OCR_SERVICE_URL, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ files: list, operations: ["enhance","deskew","contrast","denoise","ocr"] }) });
      if (response.ok) { const data: any = await response.json(); return { text: String(data.text || ""), confidence: Number(data.confidence || 0) }; }
    }
    return { text: "", confidence: 0 };
  }

  private async detectStudent(classIds: string[], text: string, suppliedLearnerId?: string | null) {
    if (suppliedLearnerId) return { learnerId: suppliedLearnerId, studentNumber: null, name: null, flags: [] as string[] };
    ...
    if (number) { const matches = learners.filter((item) => item.studentNumber.toLowerCase() === number.toLowerCase()); if (matches.length === 1) return { learnerId: matches[0].id, studentNumber: number, name: `${matches[0].firstName} ${matches[0].lastName}`, flags: [] as string[] }; return { learnerId: null, studentNumber: number, name, flags: [matches.length ? "AMBIGUOUS_STUDENT" : "STUDENT_NOT_FOUND"] }; }
    if (name) { const matches = learners.filter((item) => `${item.firstName} ${item.lastName}`.toLowerCase() === name.toLowerCase()); if (matches.length === 1) return { learnerId: matches[0].id, studentNumber: matches[0].studentNumber, name, flags: [] as string[] }; return { learnerId: null, studentNumber: null, name, flags: [matches.length ? "AMBIGUOUS_STUDENT" : "STUDENT_NOT_FOUND"] }; }
    return { learnerId: null, studentNumber: null, name: null, flags: ["STUDENT_ID_REQUIRED"] };
  }

  private detectAnswer(text: string, questionNumber: string) {
    const escaped = questionNumber.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const match = text.match(new RegExp(`(?:question\\s*)?${escaped}\\s*[).:\\-]\\s*([^\\n\\r]+)`, "i"));
    return match?.[1]?.trim() || "";
  }

  private counts(scripts: Array<{ status: string }>) { return { total: scripts.length, queued: scripts.filter((item) => ["QUEUED","OCR","MARKING"].includes(item.status)).length, ready: scripts.filter((item) => item.status === "READY_FOR_REVIEW").length, needsReview: scripts.filter((item) => item.status === "NEEDS_REVIEW").length, approved: scripts.filter((item) => item.status === "APPROVED").length, published: scripts.filter((item) => item.status === "PUBLISHED").length }; }
  private log(tenantId: string, assessmentId: string, scriptId: string | null, actorId: string | null, action: string, data?: Record<string, unknown>) { return this.prisma.teacherAssistantAudit.create({ data: { tenantId, assessmentId, scriptId, actorId, action, data: data as Prisma.InputJsonValue | undefined } }); }
}
