...
export class ShopService {
  constructor(private prisma: PrismaService, private webhooks: WebhookService) {}
  ...
  private async emitStoreWebhook(tenantId: string, integrationId: string | null, eventType: string, payload: object) {
  ...
  async getCatalog(options: {
    ...
    const { tenantId, since, page, pageSize, category, visibility, includeVariants = true, includeImages = true, status } =
      options;
    ...
    const variants = includeVariants
      ? products.flatMap((p: { variants: Array<{ priceOverride?: number | null; stockQuantity?: number | null }> }) => p.variants)
      : [];
    const prices = variants.map((v: any) => v.priceOverride).filter(Boolean);
    const stock = variants.map((v: any) => v.stockQuantity).filter(Boolean);
    const images = includeImages ? products.flatMap((p: { images: unknown[] }) => p.images) : [];

    const lastUpdatedAt = [
      ...products.map((p: { updatedAt: Date }) => p.updatedAt),
      ...categories.map((c: { updatedAt: Date }) => c.updatedAt)
    ...
    return {
      ...
      products: products.map((product: any) => ({
        ...
        primaryImage: product.images?.find((img: any) => img.isPrimary)?.url || product.images?.[0]?.url || null
      })),
      variants,
      prices,
      stock,
  ...
  markCatalogSync(integrationId: string) {
  ...
  async upsertProduct(tenantId: string, integrationId: string, input: { name: string; slug: string; categoryId?: string }) {
  ...
  async updateStock(tenantId: string, integrationId: string, variantId: string, quantity: number) {
  ...
  async createCatalog(tenantId: string, input: { name: string; slug: string }) {
    const catalog = await this.prisma.storeCatalog.create({
      data: { tenantId, name: input.name, slug: input.slug, status: "draft" }
    });
    await this.logSyncEvent(tenantId, "store.catalog.created", "StoreCatalog", catalog.id, catalog as any);
    return catalog;
  }

  async publishCatalog(tenantId: string, catalogId: string, status: string) {
  ...
  async createCategory(tenantId: string, input: any, integrationId?: string) {
    ...
    await this.logSyncEvent(tenantId, "store.category.created", "StoreCategory", category.id, category as any);
    await this.emitStoreWebhook(tenantId, integrationId || null, "store.category.created", category);
    return category;
  }

  async updateCategory(tenantId: string, id: string, input: any, integrationId?: string) {
    ...
    await this.logSyncEvent(tenantId, "store.category.updated", "StoreCategory", category.id, category as any);
    await this.emitStoreWebhook(tenantId, integrationId || null, "store.category.updated", category);
    return category;
  }

  async createProduct(tenantId: string, input: any, integrationId?: string) {
    const product = await this.prisma.storeProduct.create({
      data: {
        ...
        images: input.images?.length
          ? {
              create: input.images.map((image: any) => ({
                tenantId,
                url: image.url,
    ...
    await this.logSyncEvent(tenantId, "store.product.created", "StoreProduct", product.id, product as any);
    await this.emitStoreWebhook(tenantId, integrationId || null, "store.product.created", product);
    return product;
  }

  async updateProduct(tenantId: string, id: string, input: any, integrationId?: string) {
    ...
    await this.logSyncEvent(tenantId, "store.product.updated", "StoreProduct", product.id, product as any);
    await this.emitStoreWebhook(tenantId, integrationId || null, "store.product.updated", product);
    return product;
  }

  async duplicateProduct(tenantId: string, id: string, actorId?: string) {
  ...
  async updateProductStatus(tenantId: string, id: string, isActive: boolean, actorId?: string) {
    const existing = await this.prisma.storeProduct.findFirst({ where: { id, tenantId } });
    if (!existing) throw new NotFoundException("Product not found");
  ...
  async adjustProductStock(tenantId: string, id: string, input: any, actorId?: string) {
    ...
    if (magnitude === 0) throw new BadRequestException("Quantity must be greater than zero");
  ...
  async productHistory(tenantId: string, id: string) {
    const product = await this.prisma.storeProduct.findFirst({ where: { id, tenantId } });
    if (!product) throw new NotFoundException("Product not found");
  ...
  async deleteProduct(tenantId: string, id: string) {
    ...
    if (product._count.orderItems > 0) {
      throw new BadRequestException("Products with sales history cannot be deleted. Disable the product instead.");
    }
    await this.prisma.storeProduct.delete({ where: { id } });
    return { deleted: true };
  }

  async createOrder(tenantId: string, parentId: string, input: any) {
    if (!input.items?.length) throw new BadRequestException("Your cart is empty");
    const ids = input.items.map((item: any) => item.productId);
    const products = await this.prisma.storeProduct.findMany({ where: { tenantId, id: { in: ids }, isActive: true, allowOnlinePurchase: true } });
    if (products.length !== new Set(ids).size) throw new BadRequestException("One or more products are unavailable");
    return this.prisma.$transaction(async (tx) => {
      let totalAmount = 0;
      const lines: Array<{ product: (typeof products)[number]; quantity: number }> = input.items.map((line: any) => {
        const product = products.find((item) => item.id === line.productId)!;
        const quantity = Number(line.quantity);
        if ((product.stockQuantity ?? 0) < quantity) throw new BadRequestException(`${product.name} does not have enough stock`);
