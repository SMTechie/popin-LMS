...
export class HostelService {
  ...
  private async tenantId() { return (await this.tenants.getDefaultTenant()).id; }
  private required(value: unknown, label: string) { if (value === undefined || value === null || value === "") throw new BadRequestException(`${label} is required`); }
  ...
  async createBuilding(body: any, actorId: string) {
    this.required(body.name, "Name"); this.required(body.code, "Code"); const tenantId = await this.tenantId();
    ...
    await this.audit.log({ actorId, action: "hostel.building.create", entity: "HostelBuilding", entityId: item.id }); return item;
  }
  async createBlock(body: any, actorId: string) {
    this.required(body.buildingId, "Building"); this.required(body.name, "Name"); const tenantId = await this.tenantId();
    ...
    await this.audit.log({ actorId, action: "hostel.block.create", entity: "HostelBlock", entityId: item.id }); return item;
  }
  async createRoom(body: any, actorId: string) {
    this.required(body.blockId, "Block"); this.required(body.roomNumber, "Room number"); const tenantId = await this.tenantId(); const capacity = Number(body.capacity || 1); if (capacity < 1 || capacity > 50) throw new BadRequestException("Capacity must be between 1 and 50");
    ...
    await this.audit.log({ actorId, action: "hostel.room.create", entity: "HostelRoom", entityId: item.id, data: { capacity } }); return item;
  }

  async decideApplication(id: string, body: any, actorId: string) {
    const status = String(body.status || ""); if (!["UNDER_REVIEW", "APPROVED", "WAITLISTED", "REJECTED", "WITHDRAWN"].includes(status)) throw new BadRequestException("Invalid application status");
    ...
    await this.audit.log({ actorId, action: `hostel.application.${status.toLowerCase()}`, entity: "HostelApplication", entityId: id }); return item;
  }
  async allocate(body: any, actorId: string) {
    this.required(body.learnerId, "Learner"); this.required(body.bedId, "Bed"); const tenantId = await this.tenantId();
    return this.prisma.$transaction(async tx => {
      ...
      if (!bed || bed.status !== "AVAILABLE") throw new BadRequestException("Selected bed is not available"); if (existing) throw new BadRequestException("Learner already has an active hostel allocation");
      ...
      await tx.hostelBed.update({ where: { id: body.bedId }, data: { status: "RESERVED" } }); if (body.applicationId) await tx.hostelApplication.update({ where: { id: body.applicationId }, data: { status: "ALLOCATED" } }); return item;
  ...
  async checkIn(id: string, actorId: string) { return this.changeAllocation(id, "CHECKED_IN", actorId); }
  private async changeAllocation(id: string, status: string, actorId: string, reason?: string) {
    const current = await this.prisma.hostelAllocation.findUnique({ where: { id } }); if (!current) throw new NotFoundException("Allocation not found");
    ...
    await this.audit.log({ actorId, action: `hostel.allocation.${status.toLowerCase()}`, entity: "HostelAllocation", entityId: id }); return result[0];
  }
  async checkOut(id: string, body: any, actorId: string) { return this.changeAllocation(id, "CHECKED_OUT", actorId, body.reason); }
  async transfer(id: string, body: any, actorId: string) {
    this.required(body.bedId, "New bed"); const current = await this.prisma.hostelAllocation.findUnique({ where: { id } }); if (!current || !activeAllocation.includes(current.status)) throw new BadRequestException("Only active allocations can be transferred");
    const tenantId = await this.tenantId(); const bed = await this.prisma.hostelBed.findFirst({ where: { id: body.bedId, tenantId, status: "AVAILABLE" } }); if (!bed) throw new BadRequestException("New bed is not available");
  ...
  async parentView(parentUserId: string, learnerId: string, permissions: any) {
    const tenantId = await this.tenantId();
