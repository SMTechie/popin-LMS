...
exports.NumericMarkingPlugin = void 0;
...
class NumericMarkingPlugin {
    ...
    mark(question, detectedAnswer) {
        ...
        const expected = numeric(String(expectedRaw));
        const detected = numeric(detectedAnswer);
        if (expected === null || detected === null)
            return { awarded: 0, confidence: .45, rule: "Numerical parse failed", explanation: "Expected or detected value could not be parsed as a number; teacher review is required." };
        const tolerance = Number(question.tolerance || 0);
        const difference = Math.abs(expected - detected);
        ...
        if (difference <= tolerance && unitOk && precisionOk)
            return { awarded: question.marks, confidence: 1, rule: `Numerical tolerance ±${tolerance}${question.requiredUnit ? ` with unit ${question.requiredUnit}` : ""}`, explanation: `Difference ${difference} is within the configured tolerance.` };
        ...
        if (partial)
            return { awarded: Math.min(question.marks, Number(partial.marks || 0)), confidence: .94, rule: "Numerical partial tolerance", explanation: `Answer falls within teacher-defined partial tolerance; difference is ${difference}.` };
...
exports.NumericMarkingPlugin = NumericMarkingPlugin;
